const daftarNilai = [
  ...
  daftarNilai.forEach(kelas => {
    let total = 0;
    let count = 0;
    kelas.nilai_siswa.forEach(siswa => {
      total += siswa.nilai;
      count++;
    });
    console.log(total);
    ...
    console.log(average);
    console.log('===============================================================');
    console.log(`Rata-rata untuk kelas ${kelas.kelas}: ${average}`);
    kelas.nilai_siswa.forEach(siswa => {
      if (siswa.nilai < average) {
        console.log(`${siswa.nama} memiliki nilai ${siswa.nilai}`);
      }
    });
  });
...
for (let i = 0; i < daftarNilai.length; i++) {
  const element = daftarNilai[i];
  element.siswaAboveAvg = []
  element.siswaUnderAvg = []
  const avg = element.nilai_siswa.reduce((a, b) => a + b.nilai, 0) / element.nilai_siswa.length

  for (let j = 0; j < element.nilai_siswa.length; j++) {
    const elementNilai = element.nilai_siswa[j];

    if (elementNilai.nilai > avg) {
      element.siswaAboveAvg.push(elementNilai.nama)
    } else {
      element.siswaUnderAvg.push(elementNilai.nama)
    }
