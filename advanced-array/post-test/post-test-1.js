...
const kelasA1Sorted = kelasA1.sort(function(a, b) {
    return a.nama < b.nama ? -1 : 1
})

console.log('Daftar Nilai kelas A1',kelasA1Sorted);

let kelasA2 = []

for (let i = 0; i < daftarNilai[1].nilai_siswa.length; i++) {
    kelasA1.push(daftarNilai[1].nilai_siswa[i])
}

const kelasA2Sorted = kelasA1.sort(function(a, b) {
    return a.nama < b.nama ? -1 : 1
})

console.log('Daftar Nilai kelas A2',kelasA1Sorted);

let kelasA3 = []

for (let i = 0; i < daftarNilai[2].nilai_siswa.length; i++) {
    kelasA1.push(daftarNilai[2].nilai_siswa[i])
}

const kelasA3Sorted = kelasA1.sort(function(a, b) {
    return a.nama < b.nama ? -1 : 1
})

console.log('Daftar Nilai kelas A3',kelasA1Sorted);
...
for (let i = 0; i < daftarNilai.length; i++) {
  const grup = daftarNilai[i]
  console.log(`Nama Kelas ${grup.kelas}`);  
  grup.nama_siswa = []
  for (let j = 0; j < grup.nilai_siswa.length; j++) {
    const grupNilai = grup.nilai_siswa[j]
    grup.nama_siswa.push(grupNilai.nama)
  }
  console.log(grup.nama_siswa);
}
...
for (let i = 0; i < daftarNilai.length; i++) {
  const grup = daftarNilai[i]
  console.log(`Nama Kelas ${grup.kelas}`);  
  grup.nama_siswa = grup.nama_siswa.map((d) => {
    return d.nama
  })
  console.log(grup.nama_siswa);
}